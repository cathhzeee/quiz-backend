...
async function changePassword(request, response, next) {
  ...
  try {
    ...
    if (!oldPassword) {
      throw errorResponder(errorTypes.VALIDATION_ERROR, 'Old password is required');
    }

    // Requiring the new password
    if (!newPassword) {
      throw errorResponder(errorTypes.VALIDATION_ERROR, 'New password is required');
    }

    // Requiring the confirm new password
    if (!confirmNewPassword) {
      throw errorResponder(errorTypes.VALIDATION_ERROR, 'Confirm new password is required');
    }
    ...
    if (!isMatch) {
      throw errorResponder(errorTypes.UNPROCESSABLE_ENTITY, 'Old password is incorrect');
    }

    // New password minimal length
    ...
    if (newPassword !== confirmNewPassword) {
      throw errorResponder(errorTypes.VALIDATION_ERROR, 'New password and confirm new password do not match!');
    }
    ...
    if (!success) {
      throw errorResponder(errorTypes.UNPROCESSABLE_ENTITY, 'Failed to update password');
    }
